...
const (
	...
	// KeyCtrlEnter is Ctrl+Enter. On Windows the coninput driver collapses
	// Ctrl+Enter to KeyEnter (VK_RETURN has no modifier field), so this fork
	// distinguishes it in keyType() (see key_windows.go). The CSI-u / kitty
	// escape sequence "\x1b[13;5u" is mapped in the sequences map below.
	KeyCtrlEnter
)
...
var sequences = map[string]Key{
	...
	// kitty / CSI-u: Ctrl+Enter. "\r" (0x0d) is already mapped to KeyEnter by
	// the control-character loop in key_sequences.go, so only the CSI-u form
	// needs an explicit entry here (its Alt variant is auto-added).
	"\x1b[13;5u": {Type: KeyCtrlEnter},
}
