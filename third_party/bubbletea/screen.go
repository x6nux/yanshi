...
// Repaint forces a FULL repaint on the next render WITHOUT erasing the screen
// first (unlike ClearScreen), so there is no flicker — the renderer simply
// treats every line as changed. Use this when the diff renderer has left a
// region stale, e.g. the footer/input after scrolling a viewport: returning
// Repaint from the scroll handler makes the whole screen redraw cleanly.
func Repaint() Msg {
	return repaintMsg{}
}
