...
func keyType(e coninput.KeyEventRecord) KeyType {
	...
	case coninput.VK_RETURN:
		// bubbletea upstream drops the Ctrl modifier for Enter on Windows.
		// This fork distinguishes Ctrl+Enter (KeyCtrlEnter) so a TUI can bind
		// Enter=send and Ctrl+Enter=newline. ctrlPressed is computed at the
		// top of keyType() from ControlKeyState.
		if ctrlPressed {
			return KeyCtrlEnter
		}
		return KeyEnter
