...
const (
	...
	// also yields READONLY_CANTINIT (1288) and READONLY_DBMOVED (1032) — both
	// measured on darwin with GOMAXPROCS=2 against this very test — and
	// sqlite.Error.Code() returns the full extended code. Masking to the primary
	// code covers the whole READONLY family with one comparison, which is the
	// honest predicate: every one of them says "this attempt lost a race", not
	// "this file is broken".
	sqliteReadOnly = 8
)
...
// policy: it covers exactly the codes measured coming out of concurrent first
// opens. Widening it would start retrying real I/O failures, which is how a
// broken disk turns into a hang instead of an error.
func isTransientOpenErr(err error) bool {
	...
	// full extended code (READONLY_DBMOVED=1032, READONLY_CANTINIT=1288, …), and
	// the heal storm produces several members of that family — all with the
	// same meaning, "somebody else is mid-move; try again".
