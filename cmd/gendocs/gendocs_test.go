...
func TestSubcommandListMatchesDispatch(t *testing.T) {
	...
	want := []string{"yanshi", "serve", "chat", "exec", "app", "goal", "vcs-mcp", "init", "daemon", "schedule", "provider", "acp", "pr", "enqueue", "auth", "doctor"}
	...
	for _, required := range []string{"serve", "chat", "exec", "app", "goal", "vcs-mcp", "init", "daemon", "schedule", "provider", "acp", "pr", "enqueue", "auth", "doctor"} {
