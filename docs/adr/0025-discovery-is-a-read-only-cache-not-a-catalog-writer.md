...
3. **发现数据结构上不可能覆盖目录，因为它连目录索引长什么样都不知道**——
   `internal/llm/eino/discovery*.go` 四个文件不 import `modelcatalog.go` 定义的任何
   符号，`DiscoveredModel` 与目录的 `ModelEntry`（`modelcatalog.go`）是两个互不相关
   的类型，字段集合也不同（`DiscoveredModel` 记的是"这台机器现在有什么"，
   `ModelEntry` 记的是"这个 id 该怎么用"）。这不是靠约定维持的边界，是靠没有依赖
   关系维持的边界。
4. **人工桥接是唯一允许的桥接**：一个操作员看到 `yanshi` 探测出 LM Studio 某模型
   ...
   bootstrap 接线或 CLI 子命令去自动执行这个动作，理由见后果段落。
...
- **不可违反的约束**：**`internal/llm/eino/discovery*.go` 不得 import
  `modelcatalog.go`、`contextwindow.go`、`pricing.go` 里定义的任何导出符号**，也不得
  被这三者 import——发现子系统与目录解析子系统之间没有编译期依赖，任何一侧的重构都
  不该波及另一侧。谁要打通两者（例如让发现结果自动建议一条目录覆盖），必须先写一条
