...
- **不可违反的约束：解析出来的值贴回文本时，值里的控制算子必须仍然是数据。** 这一条是**补记的**，因为上面那两条「被否决方案」的理由（会产生不可申诉的拒绝）在被采纳的方案上**同样发生了**，只是换了一类输入，而当时的《后果》没有记：

  ```
  X='; rm -rf /'; echo $X      实测 结构性 HardDeny（yolo 也越不过），应有 Allow
  X=' && rm -rf /'; echo $X    同上
  ```

  POSIX shell **不对展开结果重新识别控制算子** —— 它只做字段切分，`;` 是 `echo` 打印的一个词。把值原样贴回文本让 `splitControlSegments` 在**数据**上切了一刀，于是一条什么都不删的命令被判成灾难档。方案 A/B 是「未解析的展开」造成不可申诉拒绝，这一条是「已解析的展开」造成的，两者是同一个代价的两面。现在的读法把带算子的字段用单引号包起来再贴回去；值里自带引号时就**不解析**，回到「只解析读得动的」这条总规则。看守是 `internal/guard::TestExpandKnownParametersResolvesOnlyWhatTheStringDefines` 里的 `X='; rm -rf /'; echo $X` 两行加语料同名行，配套正向对照是 `MSG='rm -rf /'; echo "$MSG"`。
- **字段切分是展开读法的一部分，不是可选的精细化。** `IFS` 可以在同一条命令里被重定义（`IFS=,; X=rm,-rf,/; $X`），此时 `$X` 展开成**三个词**而不是一个。原先逐字节替换的读法把程序词读成 `rm,-rf,/`，它不在任何表里，整条命令判 `DestructionNone` 而 `/bin/sh` 删了根。`${IFS}` 的值必须**变成空格而不是被丢掉**，否则 `rm${IFS}-rf${IFS}/` 会塌成 `rm-rf/`。
- **`$1`..`$9` 在同一条命令里出现过 `set --` 时必须解析。** ADR 初稿把它们与 `$$` / `$?` 并列写成「不是攻击者从字符串内部选定的」，那句话写在 `set --` 还没被读的时候；`endSegment` 记下位置参数之后，`$1` 与 `$X` 的来源完全一样。实测漏解析时 `set -- /; rm -rf $1` 判 None，而隔壁一格的 `set -- rm -rf /; "$@"` 被正确拒绝。
