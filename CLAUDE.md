...
**可覆盖 HardDeny**（`Overridable=true`）涵盖 profile 能说"不"的一切：空的 tools/fs allowlist、`shell.policy: "deny"`、`net.allow: false`、denylist 命中、execpolicy hard_deny 规则、空 MCP allowlist——这些是"profile 策略"，`yolo` 直接越过、`auto` 交给 AI 评判（详见下方模式语义）。换言之：**yolo/auto 不受 profiles 限制**（含 MCP），只受上面 5 条结构性防线 + 破坏性删除门限制。
