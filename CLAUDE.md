...
- `spawnenv_test.go`（不带 GOV 编号，`TestEverySpawnSiteDeclaresItsChildEnvironment`）：**每个起子进程的生产文件都必须一句话说明子进程看到的是什么环境。** `secproc` 的包头管的是**授权**，对**环境**一个字没说，两者曾经完全脱节 —— 九个生产文件写了 `exec.Command` 而不设 `cmd.Env`，Go 的语义是「原样继承父进程环境」，而 yanshi 的父进程环境里装着操作员的 provider API key。受影响的子进程一点都不冷门：stdio MCP server（一个由配置文件点名的二进制）、language server、`gh`、`git`、模型工具起的截图后端、剪贴板助手、启动期的版本探测 —— 每一个都拿到了 `OPENAI_API_KEY`。**这不是一次失误而是九次**，所以修一处不够：新增的 spawn 文件必须往 `spawnEnvCensus` 里写一行答案，「**故意继承，因为……**」是合法答案，沉默不是。⚠️ **机器强制的只有 key 集合**（两个方向：漏登记判红、条目指向的文件不再 spawn 也判红）；**答案是否属实机器判不了** —— 这是文本扫描不是污点分析。别拿这张表当「某个子进程已被清洗」的证据，那个证据在 `internal/netpolicy/credscrub_child_test.go`（真起子进程读回它自己的环境）。
