...
- `destructivecensus_test.go`（不带 GOV 编号，`TestDestructiveReadingConsumptionPointsAreEnumerated` / `TestDestructiveCensusEntriesAnswerTheQuestions`）：**guard 对一条 shell 命令的读法有多个消费点，而只接了一部分** —— 这是与「命令能被写进这个字符串的第几种拼法」**不同的一条轴**，且这条轴**有限可枚举**。W-B 第二批的两条 Blocking 都在它上面：写维度只读段的第一个程序词（删除维度早在三条 ADR 前就拿到了名字无关的读法），以及 shell v2 把**模型 JSON 里的 workdir** 当成破坏性删除门的边界（v1 的同一处早就写着点名这个 bug 的注释）。该测试用 `go/ast` 扫 `internal/`+`cmd/` 的非测试代码，找出**每一个带 `Shell` 字段的 `guard.Action`/`tools.PermissionRequest`/`secproc.SecureProcessSpec` 字面量**，以及**每一处调用两个读法入口**（`ClassifyDestruction` 与 `segmentWriteTargets`）的地方，要求它们与 `destructivecensus_test.go::destructiveReadingCensus` **完全对应**（新站点没登记、登记项没站点，两个方向都判失败）。**「未接」是合法答案，「没人发现」不是** —— 每条要写清三件事：破坏性边界从哪来（模型能不能放宽）、有没有把 interpreter 交出去、以及接没接（没接要写为什么可以）。配套的第二条测试拒绝「登记了但三栏都是空字符串」这种把门禁糊过去的写法。
...
- `spawnenv_test.go`（不带 GOV 编号，`TestEverySpawnSiteDeclaresItsChildEnvironment`）：**每个起子进程的生产文件都必须一句话说明子进程看到的是什么环境。** `secproc` 的包头管的是**授权**，对**环境**一个字没说，两者曾经完全脱节 —— 九个生产文件写了 `exec.Command` 而不设 `cmd.Env`，Go 的语义是「原样继承父进程环境」，而 yanshi 的父进程环境里装着操作员的 provider API key。受影响的子进程一点都不冷门：stdio MCP server（一个由配置文件点名的二进制）、language server、`gh`、`git`、模型工具起的截图后端、剪贴板助手、启动期的版本探测 —— 每一个都拿到了 `OPENAI_API_KEY`。**这不是一次失误而是九次**，所以修一处不够：新增的 spawn 文件必须往 `spawnEnvCensus` 里写一行答案，「**故意继承，因为……**」是合法答案，沉默不是。⚠️ **机器强制的是三个方向**：漏登记判红、条目指向的文件不再 spawn 判红、**以及条目答案必须逐字点名该文件里真正持有 spawn 调用的那个顶层声明**（`spawnenv_test.go::spawnSitesIn` 用 AST 算出来，`FuncDecl` 与包级 `var f = func(){}` 两种形态都认 —— `internal/clipimg` 与 `cmd/testchanged` 用的正是后者，只走 `FuncDecl` 会把它们静默漏掉）。**第三个方向是补的，补的理由与那 8 张债务表补「主体已消失」是同一条**：只查「有站点没登记」时，一个描述了**已被删除的代码路径**的条目会永远读起来像覆盖 —— `internal/tools/shell.go` 那一行描述的是 `shell_run` 经 `secproc.Launch` 的路线，而那条分支 W-B-02 就删了，文件里真正剩下的 `shellCommand`（`task_gate_run` 的）继承了全部凭据。现在改名或删掉那个声明会让条目当场变红。**答案是否属实仍然机器判不了** —— 这是 AST 上的文本扫描不是污点分析。别拿这张表当「某个子进程已被清洗」的证据，那个证据在 `internal/netpolicy/credscrub_child_test.go`（真起子进程读回它自己的环境）。

  **「起子进程」按 `spawnenv_test.go::spawnCallRe` 认得的形态算，这比「起子进程」窄。** 认：`<任意>.Command(`/`.CommandContext(`（别名容忍，`cmd/yanshi` 把 `os/exec` 导成 `osexec`）、以及 `(syscall|unix|windows|os).(Exec|ForkExec|StartProcess|CreateProcess|CreateProcessAsUser)(`（**只认这四个包名，不做别名容忍** —— 裸的 `\w+\.Exec\(` 会把 `internal/store` 里每个 `db.Exec` 都算成 spawn 点，那批永远消不掉的死条目会反过来把门禁逼松）。还认 `<任意>.Cmd{…}` 复合字面量（`spawnenv_test.go::execCmdLiteral`，在 AST 上判类型而不是加正则 —— `map[string]*exec.Cmd{}` 的类型是 `*ast.MapType`，`internal/lsp/manager.go` 里就有一个）。**它锚的是构造而不是随后的 `.Start()`**：`.Start(`/`.Run(` 在本仓是半数类型都有的方法名，按它扫会给几十个不起进程的文件索要条目，而消不掉的条目正是门禁被逼松的起点。代价是安全方向的过近似 —— 造了却从不 Start 的 Cmd 也要一行答案，全仓恰好一处（`internal/shell/childlaunch.go` 给 sandbox 用的 stand-in），而这一行正是「给它补一句 `Start()`」从此变成改答案而不是隐形新增 spawn 点的原因。

  **「不认」的那份清单是已知盲区的清点，不是穷举** —— 措辞很重要，因为本仓在这个形状上**已经栽了四次**，每次的机制都一样：作者列出了自己想得到的写法、在树里没找到反例、于是写下「这里没有这种形态」，而那句话描述的是作者的想象力不是仓库。四次分别是：AST 门禁只走 `FuncDecl`（包级 `var f = func(){}` 里的 spawn 无主）、`destructivecensus_test.go::findDestructiveReadingSites` 只认**带显式类型**的复合字面量、本检测器首版只认 `.Command(`（于是两处 `syscall.Exec` 与 `internal/shell/console_windows.go` 的 `windows.CreateProcess` 全部隐形，而后者的 unix 孪生一直在表里）、以及这一次的 `&exec.Cmd{…}` + `cmd.Start()`（Go 里第二惯用的起法，标准库文档讲 `SysProcAttr` 时给的就是它；当时树里已经有一个实例而那个文件根本不在 census）。当前已知盲区：函数值/接口方法、反射、cgo、裸 `syscall.Syscall(SYS_EXECVE, …)`、`NewLazyDLL("kernel32").NewProc("CreateProcessW").Call(…)`、以及别名成别的名字的 syscall 包。形态清单由 `spawnenv_test.go::TestSpawnCensusRecognisesEveryKnownSpawnShape` **三向**钉住（认得的各一条、`db.Exec` 这类不误伤的各一条、**已知盲区各一条**）—— 第三向能抓的只有「检测器变强了而这份清单没跟上」，抓不了没人想到过的形态，这一点写在它自己的注释里。

  **另外两处机器判不了、按约定读**：SUBJECT 方向是**声明粒度**不是调用点粒度（在已登记的声明**内部**再加一个 `cmd.Env` 为 nil 的 spawn，门禁全绿 —— 实测过；要做成调用点粒度就得让条目携带「这个声明里有几个 spawn」这种裸计数，恰好是本文件禁止的那个形状）；答案里可以点名**不存在**的声明而不变红（检查的是「每个真实声明都被点到」，不是「每个名字都真实存在」），所以答案里的名字不是「这些声明存在」的证据。两条的取舍与代价写在 `spawnenv_test.go` 文件头的 NOT ENFORCED 段。
...
**可覆盖 HardDeny**（`Overridable=true`）涵盖 profile 能说"不"的一切：空的 tools/fs allowlist、`shell.policy: "deny"`、`net.allow: false`、denylist 命中、execpolicy hard_deny 规则、空 MCP allowlist——这些是"profile 策略"，`yolo` 直接越过、`auto` 交给 AI 评判（详见下方模式语义）。换言之：**yolo/auto 不受 profiles 限制**（含 MCP），只受上面那 6 条结构性防线限制 —— **破坏性删除门就是其中第 1、2 两条，不是额外的一条**（这里此前把它并列写成「N 条 + 破坏性删除门」，读者会多数出一条）。此外越界删除（`OutOfScope`）与读不懂的 payload（`Opaque`）都不是 HardDeny 而是 Prompt，因此两者都不在这 6 条里；**`yolo` 对这两档都不自动放行** —— 越界删除它直接拦（`(deny, true)`），Opaque 它**弹窗**（`(deny, false)`，走 `req.ForcePrompt` 那条路：不自动放行、交回 callback 显式审批）。Opaque 曾经根本不在 `resolvePermissionMode` 的破坏性开关里，于是 yolo 自动批准了整档 ——「我读不出这条命令安不安全」接着「自动批准它」，等于否掉这一档存在的全部理由；理由见 [ADR-0020](docs/adr/0020-a-command-in-a-word-nobody-read.md)。⚠️ **payload 读得出灾难时档位更高** —— payload 若能被读成 shell 命令且判 Catastrophic，档位就是 Catastrophic（第 1 条，yolo 也拦），与哪个程序接收它无关。`fish -c "rm -rf /"` 因此与 `bash -c "rm -rf /"` 同判；理由见 [ADR-0019](docs/adr/0019-the-tier-follows-the-payload-not-the-program-name.md)。
...
**flag 之外还有一半：尾部 argv（`guard.classifyTrailingArgv`）。** `opaquePayload` 只对「flag 标成 code 的操作数」发声，而 `pkexec rm -rf /` / `firejail` / `bwrap` / `strace` / 一个**编造的程序名** 全部走的是无 flag 的尾部 argv，实测全 Allow 且真 shell 真跑。判据是结构性的 —— **argv 的任一后缀读得出一条破坏性命令就报**，与前面那个程序叫什么无关；`prefixRunners`/`remoteShellRunners` 里的程序按定义会跑它的 argv，判全档，其余程序**封顶 Opaque**（「它会不会执行 argv」正是未知的那一件事，`echo rm -rf /` 只是打印六个词），`scriptEmitters` 整个豁免。**这一半无论 `read` 标志如何都跑**：`taskset -c 0 rm -rf /` 曾被 prefix stripper 误读（`-c` 同时算 value flag 与 mask positional，吃掉了 `rm`），而「被认领」本身就把兜底关掉了 —— **「我读错了」拿到的判决比「我读不动」还弱**。配套的 `guard.nonInterpreterPrograms` 是过严缓解表，**漏一条 = 多一次弹窗，错放一条 = 静默放行**。理由、被否决的四个替代方案与四条不可违反的约束见 [ADR-0018](docs/adr/0018-an-unread-payload-is-a-refusal-not-a-pass.md)。判定只做词法分析（`lexShellLite`，容忍 `*`/`$`/`\`，这些恰是 execpolicy lexer 会拒掉的灾难形式）；**程序词的拼法一律看穿而不是拒绝** —— 反斜杠转义、`FOO=1` 赋值前缀、`{`/`!`/`then`/`do` 这类保留字、`eval`、以及 `sudo`/`nohup`/`timeout` 这类前缀执行器，权威清单在 `ClassifyDestruction` 自己的 doc 注释上（每一条都是实测过的 Allow，别在这里数）；含控制算子的命令**也拆段逐个判、取最严** —— 这一条是 INF1 的承重配套：它原先返回 None 把链交给 checkShell 那道整条 HardDeny，而那道兜底一细化，`ls && rm -rf /` 就会两头落空。workdir 由 shell 工具注入，未知时绝对路径按越界处理（fail-safe）。**这句原先写作「`Action.Workdir = s.root`」，只对 shell v1 成立** —— shell v2 的 `shell_start`/`task_shell_start` 把**模型 JSON 里的 `workdir`** 直接交给了这一维，于是 `{"workdir":"/","command":"rm -rf /work"}` 能把真项目根的祖先删除从**结构性 HardDeny** 降到 Allow：模型自己指定了那条判定它自己越不越界的线。现在两条路都由服务端定：v1 传 `s.root`，v2 走 `internal/tools::ShellV2Tools.guardWorkdir`（模型给的 workdir **只能收窄**——在 root 之内才采纳，其余一律回落到 root；进程**跑**在哪里仍由调用方的 `workdir` 决定，那是另一个问题）。
...
**fs 维度有三张内置 denylist，先于 profile 的 glob 跑**（`internal/guard/sensitive.go`，全部是 **Prompt + 字面授权逃生门**，不是结构性 HardDeny）：**读**（凭据：`~/.ssh`、`~/.aws`、`/etc/shadow`…，判据「读到它就拿到了通往本项目之外的密钥」）、**写**（`guard.IsExecutedOnWritePath`：`~/.bashrc`、`/etc/cron.d`、PATH 上的目录…，判据「写下去 = 延迟执行，本会话没人会读」）、以及 W-B-18 新增的**项目元数据写**（`guard.IsProtectedMetadataPath`：`.git` `.yanshi` `.agents` `.codex`，`profiles.<n>.fs.protected` 可**追加**不可删减）。第三张是**按路径段**匹配而不是按 home 后缀或绝对前缀 —— 前两张的锚点都落在可写根之外，而 `.git` 恰恰在**根之内**，正是 `fs: { write: ["**"] }` 一定会放行的位置；`.github` 因此不误伤，子模块与 vendor 目录里任意深度的 `.git` 则照样命中。逃生门与前两张同一条：在 `fs.write` 里**字面**写出那条路径（不带任何通配符）。

**模式语义（`internal/api/http::resolvePermissionMode`，仅 WS 有 callback 时生效；SSE 无 callback 一律 fail-closed）**：
- **yolo**：越过全部 profile 策略；拦 Catastrophic 与 OutOfScope 删除，并对 Opaque（读不懂的 payload）**弹窗**而不是放行。工作目录内的 `rm -rf build/` 等仍放行。
...
**这里同样只给符号名不给行号** —— 上一版写的 `tools/shell.go:171` 早已漂进 factory 分支内部，指向了它想描述的那条路径的反面。shell v2 则是**有意的另一条**路径：`shell.Manager` 用 `shell.Config.Factory`（`SecureLaunchFactory`，`internal/shell/procfactory.go`）—— 接口、spec、返回值都与 `secproc.Factory` 不同，一个类型无法同时实现两者，鉴权在**工具层**完成，分两段，别把它们混成一句：

- **每个** v2 工具都被 `NewGuardedTool` 的包装器（`internal/tools::GuardedTool.Stream`）在 handler 跑起来**之前**按自己的名字 `Authorize` 一次 —— 这是结构性的，新工具白拿。由 `internal/tools::TestShellV2EveryToolIsGatedUnderItsOwnName` 钉住：反射遍历 `ShellV2Tools` 的每个工具字段、逐个真跑，要求在「什么都不许」的 profile 下每个都被拒、且那一次问的是**它自己的名字**（新增字段自动进覆盖面）。
- **两个 launch 工具**（`shell_start` / `task_shell_start`）的 handler 另外用 `authorizeLaunch` 做一次**带 `Shell`/`Interpreter`/`Workdir` 的富 Action** 的 `Authorize` —— guard 的破坏性删除维度与 shell 维度**只能从这三个字段看见东西**，所以这一次才是承重的那次。由 `internal/tools::TestShellV2StartFeedsWorkdirToGuard` 两个 launch 各钉一遍。其余 8 个工具 handler 里那句 `Authorize` 与包装器那次**逐字段相同**，是冗余的第二道，删掉不开洞（实测：删掉 `shell_resize` 的那句，整个 tools 包全绿）。

**新增 shell v2 工具时**：包装器保证它被按名字 gate 住，但**凡是自己拼命令串下发的**（launch 形态）必须自己带上带 `Shell`/`Workdir` 的 `Authorize`，那里没有 `secproc` 兜底。

**这里原先给的是一条 `grep` 自检**（数 `NewGuardedTool(` 与 `Authorize(` 两个数、要求相等）。它有两层坏：表层是 W-B 把它变成了假指令（`shell_resize` 新增、两个 launch 合用 `authorizeLaunch`，于是 10/9），这是本文件自己警告过的裸计数腐烂；**深层是那个数从一开始量的就是冗余而不是覆盖** —— 按名字 gate 这件事根本不发生在 handler 里，而单靠数数永远看不出这一点。这是「跑起来，别读它」：把 `grep` 换成真测试的过程本身推翻了 `grep` 想表达的那句话。

**授权只是两个轴之一，另一个轴是子进程的环境。** `secproc` 的约束一个字都没说环境，而 Go 里 `cmd.Env` 留 nil 等于「原样继承父进程」，父进程环境里装着操作员的 provider API key。W-B-11 补了这条轴上的九个漏点（stdio MCP server、language server、`gh`、`git worktree`、skills 的 `git clone`、截图后端、剪贴板助手、版本探测），**漏掉了第十个**：`internal/tools/shell.go::shellCommand`（唯一调用方是 `task_gate_run`）—— 它在默认 profile 的 allow 里、命令串由**模型**写、输出进 `work.Evidence.Summary` 再被模型读回，是这批里最不该漏的一个。**而 census 对这个文件登记的答案描述的是 `shell_run`**（`shell_run` 走的那条分支在 W-B-02 就删了），于是这一行不是覆盖而是预授权。九处与第十处现在统一走 `netpolicy.ScrubbedEnviron`；它**不发布代理变量**，那是 `netpolicy.PrepareEnvFor`（经 shell.childLaunchPosture.env，`internal/shell/childlaunch.go`，供 shell_run 与 ACP agent 的发射路径调用）的活，把两者合并会让一次凭据修复顺手改掉 language server 的出网行为。当前全量 spawn 面登记在 `internal/archtest/spawnenv_test.go` 的 `spawnEnvCensus` 里（**故意继承**的那一条是 goalloop 跑项目自己的测试命令，理由写在条目上）。
