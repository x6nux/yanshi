...
**授权只是两个轴之一，另一个轴是子进程的环境。** `secproc` 的约束一个字都没说环境，而 Go 里 `cmd.Env` 留 nil 等于「原样继承父进程」，父进程环境里装着操作员的 provider API key。W-B-11 补了这条轴上的九个漏点（stdio MCP server、language server、`gh`、`git worktree`、skills 的 `git clone`、截图后端、剪贴板助手、版本探测），**漏掉了第十个**：`internal/tools/shell.go::shellCommand`（唯一调用方是 `task_gate_run`）—— 它在默认 profile 的 allow 里、命令串由**模型**写、输出进 `work.Evidence.Summary` 再被模型读回，是这批里最不该漏的一个。**而 census 对这个文件登记的答案描述的是 `shell_run`**（`shell_run` 走的那条分支在 W-B-02 就删了），于是这一行不是覆盖而是预授权。九处与第十处现在统一走 `netpolicy.ScrubbedEnviron`；它**不发布代理变量**，那是 `netpolicy.ManagedEnvWithPolicy` 的活，把两者合并会让一次凭据修复顺手改掉 language server 的出网行为。当前全量 spawn 面登记在 `internal/archtest/spawnenv_test.go` 的 `spawnEnvCensus` 里（**故意继承**的那一条是 goalloop 跑项目自己的测试命令，理由写在条目上）。
