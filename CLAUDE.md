...
**授权只是两个轴之一，另一个轴是子进程的环境。** `secproc` 的约束一个字都没说环境，而 Go 里 `cmd.Env` 留 nil 等于「原样继承父进程」，父进程环境里装着操作员的 provider API key。W-B-11 把这条轴上的九个漏点补齐（stdio MCP server、language server、`gh`、`git worktree`、skills 的 `git clone`、截图后端、剪贴板助手、版本探测），统一走 `netpolicy.ScrubbedEnviron`；它**不发布代理变量**，那是 `netpolicy.ManagedEnvWithPolicy` 的活，把两者合并会让一次凭据修复顺手改掉 language server 的出网行为。当前全量 spawn 面登记在 `internal/archtest/spawnenv_test.go` 的 `spawnEnvCensus` 里（**故意继承**的那一条是 goalloop 跑项目自己的测试命令，理由写在条目上）。
