...
**这里同样只给符号名不给行号** —— 上一版写的 `tools/shell.go:171` 早已漂进 factory 分支内部，指向了它想描述的那条路径的反面。shell v2 则是**有意的另一条**路径：`shell.Manager` 用 `shell.Config.Factory`（`SecureLaunchFactory`，`internal/shell/procfactory.go`）—— 接口、spec、返回值都与 `secproc.Factory` 不同，一个类型无法同时实现两者，鉴权改由 `internal/tools/shell_v2.go` 里注册的**每一个** v2 工具各自在工具层 `Authorize(guard.Action{...})` 完成，由 `internal/tools::TestShellV2EveryToolAuthorizesUnderItsOwnName` 钉住：它反射遍历 `ShellV2Tools` 的每个工具字段、逐个真跑一遍，要求每个工具在做任何事之前**用自己的名字**问过一次 guard（新增字段自动进覆盖面）。**新增 shell v2 工具时务必自己带上 `Authorize`**，那里没有 `secproc` 兜底。

**这里原先给的是一条 `grep` 自检**（数 `NewGuardedTool(` 与 `Authorize(` 两个数、要求相等）。W-B 把它变成了假指令：`shell_resize` 新增、而 `shell_start`/`task_shell_start` 合用 `authorizeLaunch`，于是 10/9 —— **底层不变量没破，代理破了**。这正是本文件自己警告过的裸计数问题的另一种形态：一个描述别处文件当前形状的数字，撑不过那个文件的下一次重构，而按它去查的人会去找一个并不存在的漏洞。**能写成测试的自检就不要写成 `grep`。**
